...
# ─── Режим TTS ─────────────────────────────────────────────────────────
# ⚠️ Это поле нигде не читается кодом (grep не находит ни одного импорта
# config.TTS_MODE) — просто документация текущего состояния, не переключатель.
# "edge"    — Piper ONNX локально на G1 (CPU, real-time) — НЕ РАБОТАЕТ:
#             models/burunov.onnx не обучен и не существует в репо.
# "server"  — GPT-SoVITS на внешнем GPU-сервере — НЕ РАБОТАЕТ: нет чекпоинтов.
# "presets" — РЕАЛЬНО РАБОЧИЙ путь сейчас: XTTS v2 (см. colab_xtts_v2.ipynb)
#             заранее синтезировал fixed-набор фраз в data/preset_wav/,
#             отдаются через api.py:/presets/{name}/audio (preset_audio.py).
#             Голосом Бурунова можно сказать только эти фразы — для
#             произвольного текста из RAG живого TTS нет.
TTS_MODE = "presets"
...
# 27k анекдотов на CPU индексируются ~2.7 часа; для демо — верх собственного
# же диапазона отсюда (3000-5000), а не 1500 (было ниже минимума).
# Полный датасет (None/27326) — только если есть время на переиндексацию
# (~2.7ч на CPU) или GPU под рукой (build_vector_db.py:97 сейчас device='cpu').
MAX_JOKES_FOR_INDEX = 5000
